...
class PluginModuleLoader extends Loader {
  constructor(patterns, modulesOpts, opts) {
    ...
    assert(isString(opts.key), `[${pkg.name}] PluginModuleLoader: wrong opts.key`);
    ...
    let orderedPlugins;
    if (CACHE[key]) {
      orderedPlugins = CACHE[key];
    } else {
      const plugins = loadPlugins(modulesOpts, completed);
      orderedPlugins = CACHE[key] = sortPlugins(plugins, completed);
    }
...
function loadPlugins(modulesOpts, completed = {}) {
  ...
  for (const key in profile) {
    ...
    if (pattern) {
      ...
      for (const m of modules) {
        if (m.modes && !item.modes) { plugin.modes = m.modes; }
        if (m.dependencies) { plugin.dependencies = m.dependencies; }
        if (m.optionalDependencies) { plugin.optionalDependencies = m.optionalDependencies; }
      }
    }
