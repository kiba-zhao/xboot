...
const { pkg, loadMatchedModules, resolveModule, isArray, isString, isPlainObject, isPatterns } = require('./utils');

const PLUGINS_KEY = Symbol('PLUGINS_KEY');
...
class PluginModuleLoader extends Loader {
  ...
  constructor(patterns, modulesOpts, opts = {}) {
    ...
    assert(opts.context === undefined || isPlainObject(opts.context), `[${pkg.name}] PluginModuleLoader: wrong opts.context`);

    super();

    const context = opts.context;
    let orderedPlugins;
    if (context && context[PLUGINS_KEY]) {
      orderedPlugins = context[PLUGINS_KEY];
    } else {
      const completed = {};
      const plugins = loadPlugins(modulesOpts, completed);
      orderedPlugins = plugins.length > 0 ? sortPlugins(plugins, completed) : plugins;
    }

    if (context && !context[PLUGINS_KEY]) {
      context[PLUGINS_KEY] = orderedPlugins;
    }

    this[PROPERTY_MODULE_LOADER] = orderedPlugins.length > 0 ? new ModuleLoader(patterns, orderedPlugins, { reverse: false, expand: opts.expand, mode: opts.mode }) : orderedPlugins;
  }
...
function loadPluginsProfile(modulesOpts) {
  ...
  for (const opts of modulesOpts) {
    ...
    const modules = loadMatchedModules(opts.plugin, null, opts);
    for (const m of modules) {
      if (!m || !isPlainObject(m.module)) { continue; }
      for (const key in m.module) { profile[key] = { ...m.module[key], opts }; }
    }
  }
  return profile;
}
...
function loadPlugins(modulesOpts, completed) {
  ...
  for (const key in profile) {
    ...
    const plugin = { ...item, cwd, name: key, plugin: true };
