...
function loadPlugins(modulesOpts, completed = {}) {
  ...
  for (const key in profile) {
    ...
    for (const m of modules) {
      plugin.modes = m.modes || plugins.modes;
      plugin.dependencies = m.dependencies || plugin.dependencies;
      plugin.optionalDependencies = m.optionalDependencies || plugin.optionalDependencies;
    }
