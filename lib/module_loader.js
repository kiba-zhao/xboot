...
const PROPERTY_EXPAND = Symbol('EXPAND');
...
class ModuleLoader extends Loader {
  ...
  constructor(patterns, modulesOpts, opts) {
    ...
    if (opts) {
      ...
      assert(opts.expand === undefined || isBoolean(opts.expand), `[${pkg.name}] Boot: wrong opts.expand`);
    }
    ...
    this[PROPERTY_EXPAND] = opts && opts.expand !== undefined ? opts.expand : true;
  }
  ...
  * [Symbol.iterator]() {
    ...
    const expand = this[PROPERTY_EXPAND];
    if (modulesOpts && modulesOpts.length > 0) { yield* loadMatchedModules(patterns, { expand }, ...modulesOpts); }
  }
}
